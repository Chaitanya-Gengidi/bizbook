...
function LandingPage({onLogin,onDemo}){
  const features=[
    {icon:"🧾",title:"GST Invoicing",desc:"Tax Invoice, Delivery Challan & Quotation in seconds. CGST/SGST auto-split."},
    {icon:"🎙️",title:"Voice Orders",desc:"Speak in Telugu, Hindi or English. AI creates the invoice automatically."},
    {icon:"💬",title:"WhatsApp Parser",desc:"Paste any customer message. AI reads it and builds the invoice draft."},
    {icon:"📦",title:"Stock Alerts",desc:"Real-time inventory tracking. Alerts before you run out of stock."},
    {icon:"🛡️",title:"Credit Risk AI",desc:"AI scores customer payment risk before you give credit."},
    {icon:"📊",title:"GST Assistant",desc:"Auto GSTR-1 & GSTR-3B summary. Know what to pay before filing."},
  ];
  const plans=[
    {name:"Starter",price:299,color:"#2563eb",features:["Invoicing & Stock","GST Reports","Print Invoices","WhatsApp Support"]},
    {name:"Pro",price:599,color:"#e8720c",popular:true,features:["All Starter features","AI Voice Invoice","WhatsApp Parser","Credit Risk Scoring","Priority Support"]},
    {name:"Business",price:999,color:"#d97706",features:["Multi-branch","All Pro features","Custom Domain","API Access","Dedicated Support"]},
  ];
  return(
    <div style={{background:"#0a0a0b",color:"#f0ede8",minHeight:"100vh",fontFamily:"'Sora','Nunito',sans-serif"}}>
      <style>{`
      ...
      `}</style>
      {/* NAV */}
      <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:100,background:"rgba(10,10,11,0.92)",backdropFilter:"blur(20px)",borderBottom:"1px solid #2a2a30",padding:"14px 24px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <span style={{fontWeight:900,fontSize:19,letterSpacing:-0.5}}>BizBook</span>
          <span style={{background:"rgba(232,114,12,0.15)",color:"#e8720c",fontSize:10,fontWeight:700,padding:"2px 8px",borderRadius:99,border:"1px solid rgba(232,114,12,0.3)",letterSpacing:1}}>SAAS</span>
        </div>
        <div style={{display:"flex",gap:10}}>
          <button onClick={onDemo} style={{padding:"8px 16px",borderRadius:9,border:"1px solid #2a2a30",background:"transparent",color:"#b8b4ad",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>Try Demo</button>
          <button onClick={onLogin} style={{padding:"8px 18px",borderRadius:9,border:"none",background:"#e8720c",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Login →</button>
        </div>
      </nav>
      {/* HERO */}
      <section style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center",padding:"100px 24px 60px",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",top:"25%",left:"50%",transform:"translateX(-50%)",width:500,height:500,background:"radial-gradient(circle,rgba(232,114,12,0.13) 0%,transparent 70%)",pointerEvents:"none"}}/>
        <div style={{background:"rgba(232,114,12,0.12)",border:"1px solid rgba(232,114,12,0.3)",borderRadius:99,padding:"6px 16px",fontSize:12,fontWeight:700,color:"#e8720c",letterSpacing:1,marginBottom:24,display:"inline-flex",alignItems:"center",gap:6}}>
          <span style={{animation:"pulse 2s infinite"}}>●</span> Now live in Hyderabad
        </div>
        <h1 style={{fontSize:"clamp(32px,7vw,64px)",fontWeight:900,lineHeight:1.05,letterSpacing:-2,marginBottom:20,maxWidth:780}}>
          Billing & Inventory<br/>
          <span style={{background:"linear-gradient(135deg,#e8720c,#f59e0b)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Built for Indian Shops</span>
        </h1>
        <p style={{fontSize:"clamp(14px,2.2vw,18px)",color:"#78716c",maxWidth:520,lineHeight:1.7,marginBottom:36}}>
          GST invoicing, AI voice orders, WhatsApp parser, stock alerts — everything your hardware, plywood or paint shop needs. Works in Telugu, Hindi & English.
        </p>
        <div style={{display:"flex",gap:12,flexWrap:"wrap",justifyContent:"center",marginBottom:48}}>
          <button onClick={onDemo} style={{padding:"14px 28px",borderRadius:9,border:"none",background:"#e8720c",color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:"inherit",animation:"glow 3s infinite"}}>🚀 Try Free Demo</button>
          <button onClick={onLogin} style={{padding:"14px 28px",borderRadius:9,border:"1px solid #2a2a30",background:"transparent",color:"#f0ede8",fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Sign In to Your Store →</button>
        </div>
        <div style={{display:"flex",gap:28,flexWrap:"wrap",justifyContent:"center"}}>
          {[["500+","Shops using BizBook"],["₹2Cr+","Invoices processed"],["4.9★","Average rating"],["Free","30-day trial"]].map(([v,l])=>(
            <div key={l} style={{textAlign:"center"}}><div style={{fontSize:22,fontWeight:900,color:"#e8720c",fontFamily:"monospace"}}>{v}</div><div style={{fontSize:12,color:"#6b6760",marginTop:2}}>{l}</div></div>
          ))}
        </div>
      </section>
      {/* FEATURES */}
      <section style={{padding:"70px 24px",maxWidth:1050,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:48}}>
          <div style={{fontSize:11,fontWeight:700,color:"#e8720c",letterSpacing:2,textTransform:"uppercase",marginBottom:10}}>Features</div>
          <h2 style={{fontSize:"clamp(24px,4vw,40px)",fontWeight:900,letterSpacing:-1}}>Everything your shop needs</h2>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:14}}>
          {features.map((f,i)=>(
            <div key={i} style={{background:"#141416",border:"1px solid #2a2a30",borderRadius:14,padding:"22px",transition:"all 0.2s",cursor:"default"}}
              onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(232,114,12,0.5)";e.currentTarget.style.transform="translateY(-3px)";}}
              onMouseLeave={e=>{e.currentTarget.style.borderColor="#2a2a30";e.currentTarget.style.transform="translateY(0)";}}>
              <div style={{fontSize:32,marginBottom:12,display:"inline-block",animation:`float ${3+i*0.3}s ease-in-out infinite`}}>{f.icon}</div>
              <div style={{fontWeight:800,fontSize:16,marginBottom:6}}>{f.title}</div>
              <div style={{fontSize:13,color:"#78716c",lineHeight:1.6}}>{f.desc}</div>
            </div>
          ))}
        </div>
      </section>
      {/* PRICING */}
      <section style={{padding:"70px 24px",background:"#141416",borderTop:"1px solid #2a2a30",borderBottom:"1px solid #2a2a30"}}>
        <div style={{maxWidth:950,margin:"0 auto"}}>
          <div style={{textAlign:"center",marginBottom:48}}>
            <div style={{fontSize:11,fontWeight:700,color:"#e8720c",letterSpacing:2,textTransform:"uppercase",marginBottom:10}}>Pricing</div>
            <h2 style={{fontSize:"clamp(24px,4vw,40px)",fontWeight:900,letterSpacing:-1}}>Simple, affordable plans</h2>
            <p style={{color:"#6b6760",marginTop:10,fontSize:15}}>Start free for 30 days. No credit card needed.</p>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:14,alignItems:"start"}}>
            {plans.map((p,i)=>(
              <div key={i} style={{background:"#0a0a0b",border:`2px solid ${p.popular?p.color:"#2a2a30"}`,borderRadius:14,padding:"26px",position:"relative",transform:p.popular?"scale(1.02)":"none"}}>
                ...
                <div style={{display:"flex",alignItems:"baseline",gap:4,marginBottom:16}}>
                  <span style={{fontSize:36,fontWeight:900,fontFamily:"monospace"}}>₹{p.price}</span>
                  <span style={{color:"#6b6760",fontSize:13}}>/month</span>
                </div>
                <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:22}}>
                  {p.features.map(f=><div key={f} style={{display:"flex",alignItems:"center",gap:8,fontSize:13}}><span style={{color:p.color}}>✓</span><span style={{color:"#b8b4ad"}}>{f}</span></div>)}
                </div>
                <button onClick={onLogin} style={{width:"100%",padding:"11px",borderRadius:9,border:`1.5px solid ${p.color}`,background:p.popular?p.color:"transparent",color:p.popular?"#fff":p.color,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
                  Get Started
                </button>
              </div>
            ))}
      ...
      {/* CTA */}
      <section style={{padding:"60px 24px",textAlign:"center",background:"linear-gradient(135deg,rgba(232,114,12,0.1),rgba(245,158,11,0.05))"}}>
        <h2 style={{fontSize:"clamp(22px,4vw,36px)",fontWeight:900,letterSpacing:-1,marginBottom:10}}>Ready to modernise your shop?</h2>
        <p style={{color:"#6b6760",marginBottom:24,fontSize:15}}>Join 500+ shop owners. Setup in 10 minutes.</p>
        <button onClick={onDemo} style={{padding:"15px 34px",borderRadius:9,border:"none",background:"#e8720c",color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>Try Free Demo 🚀</button>
        <div style={{marginTop:14,fontSize:13,color:"#6b6760"}}>No credit card · No install · Works on any phone</div>
      </section>
      {/* FOOTER */}
      <footer style={{padding:"28px 24px",borderTop:"1px solid #2a2a30",textAlign:"center"}}>
        ...
        <div style={{fontSize:12,color:"#6b6760"}}>Made with ❤️ for Indian shopkeepers · Hyderabad · contact@bizbookhyd.in</div>
      </footer>
    </div>
  );
}
...
function LoginPage({onLogin,onBack,onDemo}){
  ...
  const handleLogin=async()=>{
    ...
    if(shopId.toLowerCase()==="svplywood"&&password==="bizbook123"){
      ...
      setError("Invalid credentials. Demo: svplywood / bizbook123");
    }
    setLoading(false);
  };

  const handleRegister=async()=>{
    if(!reg.shopName||!reg.phone){setError("Shop name and phone required");return;}
    setLoading(true);await new Promise(r=>setTimeout(r,1200));setLoading(false);setRegistered(true);
  };

  const inp={width:"100%",padding:"12px 14px",borderRadius:9,border:"1.5px solid #2a2a30",background:"#0a0a0b",color:"#f0ede8",fontSize:15,fontFamily:"inherit",outline:"none",boxSizing:"border-box"};

  return(
    <div style={{minHeight:"100vh",background:"#0a0a0b",display:"flex",fontFamily:"'Sora','Nunito',sans-serif"}}>
      ...
      <div style={{flex:1,background:"linear-gradient(160deg,#1a0a00,#0a0a0b)",display:"flex",flexDirection:"column",justifyContent:"center",padding:"60px 48px",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",top:0,left:0,right:0,bottom:0,background:"radial-gradient(ellipse at 30% 50%,rgba(232,114,12,0.15) 0%,transparent 60%)",pointerEvents:"none"}}/>
        <button onClick={onBack} style={{background:"none",border:"none",color:"#6b6760",cursor:"pointer",fontSize:13,fontFamily:"inherit",marginBottom:40,textAlign:"left"}}>← Back to home</button>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:36}}>
          <div style={{width:46,height:46,background:"#e8720c",borderRadius:13,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:22,color:"#fff"}}>B</div>
          <div><div style={{fontWeight:900,fontSize:22,color:"#f0ede8"}}>BizBook</div><div style={{fontSize:11,color:"#e8720c",fontWeight:700,letterSpacing:1}}>SHOP MANAGER</div></div>
        </div>
        <h2 style={{fontSize:30,fontWeight:900,color:"#f0ede8",lineHeight:1.2,marginBottom:14}}>Your shop.<br/>Your data.<br/>Your growth.</h2>
        <p style={{color:"#6b6760",fontSize:14,lineHeight:1.7,maxWidth:320,marginBottom:32}}>Invoicing, inventory, GST, AI assistant — all in one place. Works offline on any phone.</p>
        {["GST-ready invoices in seconds","AI in Telugu, Hindi & English","Works offline on any phone","Your data stays private & secure"].map(f=>(
          <div key={f} style={{display:"flex",alignItems:"center",gap:10,fontSize:14,color:"#b8b4ad",marginBottom:10}}>
            <span style={{color:"#22c55e",fontSize:15}}>✓</span>{f}
          </div>
        ))}
        <div style={{marginTop:32,background:"rgba(232,114,12,0.1)",border:"1px solid rgba(232,114,12,0.25)",borderRadius:10,padding:"14px 16px"}}>
          <div style={{fontSize:11,fontWeight:700,color:"#e8720c",marginBottom:6,letterSpacing:1}}>🎯 DEMO CREDENTIALS</div>
          <div style={{fontSize:13,color:"#b8b4ad",fontFamily:"monospace"}}>Shop ID: <strong style={{color:"#f0ede8"}}>svplywood</strong></div>
          <div style={{fontSize:13,color:"#b8b4ad",fontFamily:"monospace"}}>Password: <strong style={{color:"#f0ede8"}}>bizbook123</strong></div>
        </div>
      </div>
      {/* Right form panel */}
      <div style={{width:440,background:"#141416",display:"flex",flexDirection:"column",justifyContent:"center",padding:"48px 36px",borderLeft:"1px solid #2a2a30",overflowY:"auto"}}>
        <div style={{display:"flex",background:"#0a0a0b",borderRadius:9,padding:3,marginBottom:24}}>
          {["login","register"].map(t=>(
            <button key={t} onClick={()=>{setTab(t);setError("");setRegistered(false);}} style={{flex:1,padding:"9px",borderRadius:7,border:"none",background:tab===t?"#141416":"transparent",color:tab===t?"#f0ede8":"#6b6760",fontWeight:tab===t?700:500,fontSize:13,cursor:"pointer",fontFamily:"inherit",textTransform:"capitalize"}}>
        ...
        {tab==="login"?(
          <div>
            ...
            <p style={{color:"#6b6760",fontSize:13,marginBottom:24}}>Sign in to your BizBook store</p>
            <div style={{marginBottom:14}}><label style={{display:"block",fontSize:11,fontWeight:700,color:"#6b6760",marginBottom:5,textTransform:"uppercase",letterSpacing:0.6}}>Shop ID</label><input value={shopId} onChange={e=>setShopId(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleLogin()} placeholder="e.g. svplywood" style={inp}/></div>
            <div style={{marginBottom:16}}><label style={{display:"block",fontSize:11,fontWeight:700,color:"#6b6760",marginBottom:5,textTransform:"uppercase",letterSpacing:0.6}}>Password</label><input type="password" value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleLogin()} placeholder="Enter password" style={inp}/></div>
            {error&&<div style={{color:"#f87171",fontSize:13,marginBottom:14,padding:"10px 14px",background:"rgba(248,113,113,0.08)",borderRadius:9,border:"1px solid rgba(248,113,113,0.2)"}}>{error}</div>}
            <button onClick={handleLogin} disabled={loading} style={{width:"100%",padding:"13px",borderRadius:9,border:"none",background:"#e8720c",color:"#fff",fontSize:14,fontWeight:700,cursor:loading?"not-allowed":"pointer",fontFamily:"inherit",opacity:loading?0.7:1,marginBottom:12}}>
              {loading?"Signing in…":"Sign In →"}
            </button>
            <button onClick={onDemo} style={{width:"100%",padding:"12px",borderRadius:9,border:"1px solid #2a2a30",background:"transparent",color:"#b8b4ad",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>
          ...
          <div style={{textAlign:"center",padding:"20px 0"}}>
            ...
            <h3 style={{fontSize:20,fontWeight:900,color:"#f0ede8",marginBottom:8}}>Registration Successful!</h3>
            <p style={{color:"#6b6760",fontSize:13,lineHeight:1.7,marginBottom:20}}>Your BizBook store is being set up. You'll receive your Shop ID and login on WhatsApp within 10 minutes.</p>
            <div style={{background:"rgba(232,114,12,0.1)",border:"1px solid rgba(232,114,12,0.25)",borderRadius:9,padding:"12px",marginBottom:20,fontSize:13,color:"#b8b4ad"}}>
              📱 WhatsApp: <strong style={{color:"#e8720c"}}>+91 98765 43210</strong>
            </div>
            <button onClick={()=>{setTab("login");setRegistered(false);}} style={{width:"100%",padding:"12px",borderRadius:9,border:"none",background:"#e8720c",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Go to Sign In</button>
          </div>
        ):(
          <div>
            <h2 style={{fontSize:22,fontWeight:900,color:"#f0ede8",marginBottom:4}}>Register your shop</h2>
            <p style={{color:"#6b6760",fontSize:13,marginBottom:20}}>30-day free trial · No credit card</p>
            {[["Shop Name","shopName","e.g. Ravi Hardware Store"],["Owner Name","ownerName","Your full name"],["Phone (WhatsApp)","phone","10-digit number"],["City","city","Hyderabad"]].map(([label,key,ph])=>(
              <div key={key} style={{marginBottom:11}}><label style={{display:"block",fontSize:11,fontWeight:700,color:"#6b6760",marginBottom:4,textTransform:"uppercase",letterSpacing:0.6}}>{label}</label><input value={reg[key]} onChange={e=>setReg({...reg,[key]:e.target.value})} placeholder={ph} style={{...inp,fontSize:14}}/></div>
            ))}
            <div style={{marginBottom:11}}><label style={{display:"block",fontSize:11,fontWeight:700,color:"#6b6760",marginBottom:4,textTransform:"uppercase",letterSpacing:0.6}}>Shop Type</label>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
              {[["Starter","₹299/mo","#2563eb"],["Pro","₹599/mo","#e8720c"]].map(([name,price,color])=>(
                <button key={name} onClick={()=>setReg({...reg,plan:name})} style={{padding:"10px",borderRadius:9,border:`1.5px solid ${reg.plan===name?color:"#2a2a30"}`,background:reg.plan===name?color+"18":"transparent",color:reg.plan===name?color:"#6b6760",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
                  {name}<br/><span style={{fontSize:11,fontWeight:400}}>{price}</span>
                </button>
              ))}
            </div>
            {error&&<div style={{color:"#f87171",fontSize:13,marginBottom:12,padding:"10px 14px",background:"rgba(248,113,113,0.08)",borderRadius:9}}>{error}</div>}
            <button onClick={handleRegister} disabled={loading} style={{width:"100%",padding:"13px",borderRadius:9,border:"none",background:"#e8720c",color:"#fff",fontSize:14,fontWeight:700,cursor:loading?"not-allowed":"pointer",fontFamily:"inherit",opacity:loading?0.7:1}}>
              {loading?"Setting up store…":"Register & Start Free Trial →"}
            </button>
          </div>
        )}
      </div>
